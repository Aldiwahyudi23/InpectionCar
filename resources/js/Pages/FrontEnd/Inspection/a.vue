...
const activeCategory = ref('vehicle');

// PERBAIKAN: Gunakan semua appMenus (menu tetap muncul semua)
const appMenus = computed(() => {
  return props.appMenus || [];
});

// Filter points berdasarkan kondisi
const getVisiblePoints = (menuPoints, isDamageMenu) => {
  if (!isDamageMenu) {
    // Untuk menu biasa, tampilkan semua points
    return menuPoints || [];
  }
  
  // PERBAIKAN PENTING: Untuk damage menu, hanya tampilkan points yang sudah ada datanya
...
const hasPointData = (pointId) => {
  ...
  const hasServerResult = props.existingResults[pointId] !== undefined;
  
  // Cek di existingImages (data dari server)
  const hasServerImages = props.existingImages[pointId] && props.existingImages[pointId].length > 0;
...
const activeMenuData = computed(() => {
  ...
  return {
    ...menu,
    isDamageMenu: menu.input_type === 'damage',
    // PERBAIKAN: Filter points berdasarkan jenis menu
...
const initializeForm = () => {
  ...
  // Inisialisasi dari semua points
  const allPoints = [
...
const isMenuComplete = (menu) => {
  if (menu.input_type === 'damage') {
    // Untuk damage menu, cek apakah ada setidaknya satu point yang memiliki data
  ...
  // Untuk menu biasa, cek kelengkapan semua points
      ...
      await router.post(route('inspections.delete-result'), {
        ...
        onSuccess: () => {
          if (form.results[pointId]) {
            form.results[pointId].status = '';
            form.results[pointId].note = '';
          }
...
watch(searchQuery, debounce(() => {}, 300));
