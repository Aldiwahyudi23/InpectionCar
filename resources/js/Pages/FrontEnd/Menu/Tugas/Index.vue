...
const isPaid = computed(() => props.tasks?.transaction?.status === 'paid');
const isPending = computed(() => props.tasks?.transaction?.status === 'pending');
                    ...
                    >
                        ...
                        <div class="px-4 py-3 bg-gray-50 border-t border-gray-100">
                            ...
                            <div 
                                ...
                                :class="{
                                'bg-green-50 border border-green-200': task.customer,
                                'bg-yellow-50 border border-yellow-200': !task.customer
                                }"
                            >
                            ...
                            <div 
                                ...
                                'bg-red-50 border border-red-200': !task?.transaction.status === 'paid',
                            ...
                            >
                                ...
                                <span :class="{
                                    ...
                                    'text-red-600 font-semibold': !task?.transaction.status === 'paid' && !task?.transaction?.status === 'pending'
                                }">
                                    {{ statusTranslations[task.transaction.status] }}
                                </span>
                                <span>Nominal:</span>
                                <span :class="{
                                    'text-green-700': task?.transaction.status === 'paid',
                                    'text-yellow-700': task?.transaction?.status === 'pending',
                                    'text-red-700': !task?.transaction.status === 'paid' && !task?.transaction?.status === 'pending'
                                }">
                                    {{new Intl.NumberFormat('id-ID').format(task.transaction.amount) }}
                                </span>
                                <span v-if="task.transaction.payment_method">Metode:</span>
                                <span 
                                    v-if="task.transaction.payment_method"
                                    :class="{
                                    'text-green-700': task?.transaction.status === 'paid',
                                    'text-yellow-700': task?.transaction?.status === 'pending',
                                    'text-red-700': !task?.transaction.status === 'paid' && !task?.transaction?.status === 'pending'
                                ...
                                >
                                    {{ task.transaction.payment_method}}
                                </span>
                                </div>
                            </div>
                            <div 
